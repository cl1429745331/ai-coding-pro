...
(
    ...
    800,
    'Canon EOS R6',
    'Canon RF 16-35mm f/2.8 L IS USM',
    'f/8.0',
    '1/250s',
    200,
    16,
    '北京市朝阳区望京街道',
    '2024-07-12 11:20:00+08:00',
    true,
    278,
    54,
    9,
...
-- 7. 更新统计数据（触发器会自动处理，这里手动确保数据一致性）
UPDATE photos SET 
    like_count = (SELECT COUNT(*) FROM likes WHERE photo_id = photos.id),
    comment_count = (SELECT COUNT(*) FROM comments WHERE photo_id = photos.id AND is_approved = true);

UPDATE tags SET 
    photo_count = (SELECT COUNT(*) FROM photo_tags WHERE tag_id = tags.id);

UPDATE categories SET 
    photo_count = (SELECT COUNT(*) FROM photos WHERE category = categories.name AND is_published = true);

-- 8. 插入系列数据（可选）
INSERT INTO series (title, description, is_published, sort_order) VALUES
('中国山水系列', '记录中国各地的山水风光，展现祖国大好河山的壮美', true, 1),
('城市印象系列', '用镜头记录现代都市的繁华与变迁', true, 2),
('人文纪实系列', '关注普通人的生活状态，记录时代变迁中的人文故事', true, 3);

-- 9. 插入照片-系列关联数据
INSERT INTO photo_series (photo_id, series_id, sort_order) VALUES
((SELECT id FROM photos WHERE title = '晨雾中的桂林山水'), (SELECT id FROM series WHERE title = '中国山水系列'), 1),
((SELECT id FROM photos WHERE title = '海边日落的金色时光'), (SELECT id FROM series WHERE title = '中国山水系列'), 2),
((SELECT id FROM photos WHERE title = '城市夜景的璀璨灯火'), (SELECT id FROM series WHERE title = '城市印象系列'), 1),
((SELECT id FROM photos WHERE title = '现代建筑的几何美学'), (SELECT id FROM series WHERE title = '城市印象系列'), 2),
((SELECT id FROM photos WHERE title = '老街巷的午后光影'), (SELECT id FROM series WHERE title = '人文纪实系列'), 1),
((SELECT id FROM photos WHERE title = '藏族老人的深邃眼神'), (SELECT id FROM series WHERE title = '人文纪实系列'), 2),
((SELECT id FROM photos WHERE title = '雨后街道的宁静时刻'), (SELECT id FROM series WHERE title = '人文纪实系列'), 3);

-- 完成数据插入
SELECT '✅ 测试数据插入完成！' as status;
SELECT '📊 数据统计：' as info;
SELECT COUNT(*) as 照片数量 FROM photos WHERE is_published = true;
SELECT COUNT(*) as 标签数量 FROM tags;
SELECT COUNT(*) as 分类数量 FROM categories;
SELECT COUNT(*) as 评论数量 FROM comments WHERE is_approved = true;
SELECT COUNT(*) as 点赞数量 FROM likes; 